import type { CharacterModel, AbilityKey, SkillProficiency, CharacterFeat, CharacterSpell, CharacterAttack, CharacterEquipment, ProficiencyRank } from "../model/character";
...
function mapFeatType(raw: string, fallback: CharacterFeat["type"]): CharacterFeat["type"] {
  ...
  if (s.includes("class") || s.includes("monk feat") || s.includes("fighter feat") || s.includes("rogue feat")) return "class";
...
export function parsePathbuilder(json: unknown): CharacterModel {
  ...
  // --- Abilities ---
  const abilitiesRaw = (get<Record<string, unknown>>(build, "abilities") ?? {}) as Record<string, unknown>;
  ...
  // --- Skills ---
  const skillsRaw = (get<Record<string, unknown>>(build, "proficiencies") ?? {}) as Record<string, unknown>;
  const SKILL_NAMES = [
  ...
  for (const caster of spellCasters) {
    ...
    const isFocus = get<boolean>(caster, "focusSpells") === true ||
  ...
  const saves = {
    fortitude: num(get(build, "saves", "fortitude")) ??
      num(get(build, "proficiencies", "fortitude")),
    reflex: num(get(build, "saves", "reflex")) ??
      num(get(build, "proficiencies", "reflex")),
    will: num(get(build, "saves", "will")) ??
      num(get(build, "proficiencies", "will")),
  };
